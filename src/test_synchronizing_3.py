...
def test_synchronizing(model_architecture, num_models, data:list, legend:list, latent_list, epochs):
  ...
  fullsyn_auc = {'number of Autoencoders': list(range(1, num_models + 1)),
                 'satellite': fullsyn_dict['auc'][0],
                 'waveform': fullsyn_dict['auc'][1]}  
  # save result to .csv and export it
  fullsyn_df = pd.DataFrame.from_dict(fullsyn_auc)
  fullsyn_df.to_csv(DATA_PATH_OUTPUT / 'synchronizing' / 'sat_wav_20models_synchronizing.csv')
  return fullsyn_df

#LATENT_LIST = [21, 112, 192, 112, 16, 96, 32, 112, 112]
test_synchroniying = test_synchronizing(Autoencoder, num_models=10, data=[satellite, waveform], legend=['satellite', 'waveform'], latent_list=[192, 112], epochs=2000)
