# Training an ensemble of 10 different full_sequential models, in which each full_sequential model contains 10 different Autoencoders
# These full_sequential models are differentiated by random seed
...
def full_sequential(model_architecture, num_models, latent_list: list, data:list, legend:list, seed_list: list, epochs):
  ...
  # extract AUC scores of each dataset from full_sequential_dict
  cardio_auc = [full_sequential_dict['auc'][i][0] for i in range(0, 10)]
  gas_drift_auc = [full_sequential_dict['auc'][i][1] for i in range(0, 10)]
  satellite_auc = [full_sequential_dict['auc'][i][2] for i in range(0, 10)]  
  magic_telescope_auc = [full_sequential_dict['auc'][i][3] for i in range(0, 10)]
  pendigits_auc = [full_sequential_dict['auc'][i][4] for i in range(0, 10)]
  phoneme_auc = [full_sequential_dict['auc'][i][5] for i in range(0, 10)]  
  pollen_auc = [full_sequential_dict['auc'][i][6] for i in range(0, 10)]
  speech_auc = [full_sequential_dict['auc'][i][7] for i in range(0, 10)]
  waveform_auc = [full_sequential_dict['auc'][i][8] for i in range(0, 10)] 
  ...
  full_se = {'cardio mean': cardio_mean,
  ...
  plt.legend(loc='best')
  plt.savefig(DATA_PATH_OUTPUT / 'ensemble_offullsequential.jpg')
  plt.show()
  ...
  full_sequential_df = pd.DataFrame.from_dict(full_se)
  full_sequential_df.to_csv(DATA_PATH_OUTPUT / 'Ensemble_OfFullSequential.csv')  
  return full_sequential_df  

SEED_LIST = [1, 3, 42, 7, 10, 100, 123, 9, 50, 142]
full_sequential_df = full_sequential(Autoencoder, 10, LATENT_LIST, DATA, LEGEND,seed_list = SEED_LIST, epochs = 2000)
