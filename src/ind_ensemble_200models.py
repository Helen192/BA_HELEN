...
def plot_auc_models(model_architecture, num_models_list, latent_list: list, data:list, legend:list, epochs):
  ...
  num_auto_auc = {'number of Autoencoders': num_models_list,
                'satellite': num_auto['auc'][0]}

  # save result to .csv and export it
  num_auto_df = pd.DataFrame.from_dict(num_auto_auc)
  num_auto_df.to_csv(DATA_PATH_OUTPUT / 'satellite_200models_ensemble_number_of_auto.csv')
  return num_auto

po_spe_latent = [192]
num_auto_2 = plot_auc_models(Autoencoder,num_models_list=[i for i in range(1, 201)], latent_list = po_spe_latent, data = [satellite], legend = ['satellite'], epochs = 2000)
