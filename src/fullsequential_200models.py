...
def full_sequential(model_architecture, num_models, latent_list: list, data:list, legend:list, epochs):
  ...
  sesyn_df.to_csv(DATA_PATH_OUTPUT / 'rand42_200models_fullsequential.csv')
  return sesyn_df


full_sequential_df = full_sequential(Autoencoder, num_models = 200, latent_list= LATENT_LIST, data = DATA, legend = LEGEND, epochs = 2000)
