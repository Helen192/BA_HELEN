...
@cache
def subtrain(SEED, model_architecture, latent,batch, epochs, dataset_name):
...
def ensemble_model(model_architecture, seed_list, X_train, X_test, y_test, latent, epochs, dataset_name ,plot=True):
  """
  ...
  for SEED in seed_list:
    # Set random seed
    pred_errors=subtrain(SEED, model_architecture, latent,64, epochs, dataset_name)
...
def plot_auc_models(model_architecture, num_models_list, latent_list: list, data:list, legend:list, epochs):
  ...
  for n in range(len(data)):
    ...
    AUC_LIST = []
#
    base_seed=np.random.choice(70,size=np.max(num_models_list),replace=False)
    ...
    for i in num_models_list:
      ...
      auc_score = ensemble_model(model_architecture, seed_list, X_train, X_test, y_test, latent_list[n], epochs, legend[n],plot=False)
  ...
  #num_auto_df.to_csv(DATA_PATH_OUTPUT / 'ensemble_number_of_auto.csv')
  num_auto_df.to_csv(DATA_PATH_OUTPUT / 'rand500_optimal_to60 models_ensemble_number_of_auto.csv')
  return num_auto


#num_auto = plot_auc_models(Autoencoder,num_models_list=[4, 6, 8, 10, 12, 14, 16], latent_list = LATENT_LIST, data = DATA, legend = LEGEND, epochs = 2000)
#print(num_auto)


# Test with more models
#num_auto_2 = plot_auc_models(Autoencoder,num_models_list=[i for i in range(4, 61, 2)], latent_list = LATENT_LIST, data = DATA, legend = LEGEND, epochs = 2000)

#unoptimal_latent = [4, 16, 16, 4, 4, 4, 4, 256, 32]
num_auto_2 = plot_auc_models(Autoencoder,num_models_list=[i for i in range(1, 61)], latent_list = LATENT_LIST, data = DATA, legend = LEGEND, epochs = 1)
