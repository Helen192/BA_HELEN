...
def plot_auc_models(model_architecture, num_models_list, latent_list: list, data:list, legend:list, epochs):
  ...
  plt.savefig(DATA_PATH_OUTPUT / 'phon_poll_ensemble_number_of_auto.jpg')
  plt.show()
  ...
  num_auto_auc = {'number of Autoencoders': num_models_list,
                'phoneme': num_auto['auc'][0],
                'pollen': num_auto['auc'][1]}
  ...
  num_auto_df.to_csv(DATA_PATH_OUTPUT / 'phon_poll_ensemble_number_of_auto.csv')
  return num_auto
...
num_auto_2 = plot_auc_models(Autoencoder,num_models_list=[4, 6, 8, 10, 12, 14, 16], latent_list = [320, 160], data = [phoneme, pollen], legend = ['phoneme', 'pollen'], epochs = 2000)
print(num_auto_2)