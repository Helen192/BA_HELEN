...
def full_sequential(model_architecture, num_models, latent_list: list, data:list, legend:list, epochs):
  ...
  for n in range(len(data)):
    ...
    for i in range(num_models):
      ...
      avg_preds_notscaler = tf.math.reduce_mean(pred_errors, axis=1)   # shape = (X_train.shape[1], 1)
      ...
      avg_preds_test_notscaler = tf.math.reduce_mean(pred_errors_test, axis=1)   # shape = (X_train.shape[1], 1)

      # scaler applied for both avg_preds of training and test sets
      #avg_preds_scaler, avg_preds_test_scaler = scaler(avg_preds_notscaler, avg_preds_test_notscaler, scaler="minmax")

      sc_train = MinMaxScaler(feature_range=(0, 1))
      sc_test = MinMaxScaler(feature_range=(0, 1))
      avg_preds_scaler = sc_train.fit_transform(tf.expand_dims(avg_preds_notscaler, axis=1))
      avg_preds_test_scaler = sc_test.fit_transform(tf.expand_dims(avg_preds_test_notscaler, axis=1))

      # multipying the last scaled feature with scaling_factor value -> to make sure the new feature added for the next iteration will have an effect even though how many features the dataset has
      avg_preds = tf.squeeze(avg_preds_scaler * (scaling_factor(X_train) /(i +1)))
      avg_preds_test = tf.squeeze(avg_preds_test_scaler * (scaling_factor(X_train) / (i+1)))


      avg_pred_errors_list = [j * (i /(i+1)) for j in avg_pred_errors_list]
      test_errors_list = [j * (i /(i+1)) for j in test_errors_list]
  ...
  sesyn_df.to_csv(DATA_PATH_OUTPUT / 'new_unoptimal_version_60models_full_sequential.csv')
  return sesyn_df
...
full_sequential_df = full_sequential(Autoencoder, num_models = 60, latent_list= unoptimal_latent, data = DATA, legend = LEGEND, epochs = 1000)
