...
def plot_auc_models(model_architecture, num_models_list, latent_list: list, data:list, legend:list, epochs):
  ...
  for n in range(len(data)):
    ...
    AUC_LIST = []
#
    base_seed=np.random.choice(200,size=np.max(num_models_list),replace=False)
  ...
  num_auto_df.to_csv(DATA_PATH_OUTPUT / 'rand200_optimal_to60 models_ensemble_number_of_auto.csv')
  return num_auto
...
num_auto_2 = plot_auc_models(Autoencoder,num_models_list=[i for i in range(1, 61)], latent_list = LATENT_LIST, data = DATA, legend = LEGEND, epochs = 2000)
