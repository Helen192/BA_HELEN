...
def sequential(model_architecture, num_models, latent_list: list, data:list, legend:list, epochs):
  ...
  for n in range(len(data)):
    ...
    for i in range(num_models):
      ...
      #avg_preds_scaler, avg_preds_test_scaler = scaler(tf.expand_dims(avg_preds_notscaler, axis=0), tf.expand_dims(avg_preds_test_notscaler, axis=0), scaler="minmax")
      ...
      avg_preds = tf.squeeze(avg_preds_scaler * scaling_factor(X_train)) 
      avg_preds_test = tf.squeeze(avg_preds_test_scaler * scaling_factor(X_train))

      # append avg_preds (training set) to the avg_pred_errors_list
      avg_pred_errors_list.append(avg_preds)

      # append avg_preds_test (test set) to the test_errors_list
  ...
  dropsyn_df.to_csv(DATA_PATH_OUTPUT / 'new_unoptimal_version_60models_sequential.csv')  
  return dropsyn_df
